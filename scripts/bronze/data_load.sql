/*
 =============================================================
 Script: Load Bronze Layer Data (Source -> Bronze)
=============================================================
 Script Purpose:
     This script loads data into the 'bronze' database from
     external CSV files. It performs the following actions:
     - Calls bronze.load_bronze() procedure to truncate tables.
  ...
  Note:
    ...
    mysql -u root -p --local-infile=1 < data_load.sql.This is 
...
-- Usage Example:
--     mysql -u root -p --local-infile=1 < data_load.sql
-- =============================================================
*/
...
SELECT CONCAT('>>Loading Bronze Layer is Comleted - Total Load Duration: ',TIMESTAMPDIFF(SECOND,@Start_time,@end_time), ' seconds') AS ''; 
...
mysql -u root -p -e "
SELECT 'crm_cust_info' AS table_name, COUNT(*) AS row_count FROM bronze.crm_cust_info UNION ALL
