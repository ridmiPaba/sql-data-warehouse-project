...
- All gold layer objects are implemented as **views** in MySQL, built on top of the cleaned silver layer tables.
- Surrogate keys (`customer_key`, `product_key`) are generated using `ROW_NUMBER()` window functions.
- Natural keys (`customer_id`, `product_id`, `product_number`) are preserved for traceability back to source systems.
- The original tutorial uses SQL Server `NVARCHAR(50)`; this MySQL implementation uses the equivalent `VARCHAR(50)`.
